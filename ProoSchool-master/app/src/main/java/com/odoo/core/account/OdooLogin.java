            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    if (!mRequestedForAccount) {
                        Boolean isFirstRun = getSharedPreferences("PREFRENCE", MODE_PRIVATE).getBoolean("isfirstrun", true);
                        if (isFirstRun) {
                            Intent intent = new Intent(OdooLogin.this,slider.class);
                            startActivity(intent);

                            Toast.makeText(OdooLogin.this, "This is  The first time access !",
                                    Toast.LENGTH_LONG).show();
                        }
                        else {
                            Intent intent1 = new Intent(OdooLogin.this, OdooActivity.class);
